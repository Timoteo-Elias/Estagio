...
<?php 
    ...
    $sql = "SELECT v.id ,v.instituto, e.nome, e.bi, e.curso, e.nivel
    FROM valontarios v inner join estagiarios e on v.idestagiario=e.id";
...
<div class="container w-75 d-flex justify-content-between mt-5 rounded-3" style="background-color:#ffffff">
    ...
    <div class="container px-4 py-3" id="featured-3">
        <div class="text-center">
            <h3 class="">Estagiarios Volontarios</h3> 
        </div>
        <div class="d-flex justify-content-between mt-5">
            <form action="estavolonta.php" method="post" class=" d-flex" role="search">
                <input name="pesquisa" class="form-control me-2" type="search" placeholder="Buscar pelo Nome" aria-label="Search">
                <button class="btn btn-outline-success" type="submit"><i class="bi bi-search"></i></button>
            </form>
            <div>
                <a class="btn btn-outline-danger" target="_blank" href="estavolontapdf.php"><i class="bi bi-file-pdf-fill"></i></a>
...
<?php 
    ...
    if(isset($_POST) && !empty($_POST))
    {
        ...
        $query = mysqli_query($con, "SELECT * FROM valontarios WHERE idestagiario='$estagiario'");
        ...
        if ($query) {
            ...
            if ($result > 0) {
                ?>
                <script>
                    alert("As Credenciais inseridas já existem. Consulte a Lista!!");
                </script>
                <?php
            }
            else{
                $sql = mysqli_query($con, "INSERT INTO valontarios(idestagiario,instituto) 
                VALUES('$estagiario','$instituto')");
                if($sql==1)
                { 
                    ?>
                        <script>
    ...
    <div class="modal-dialog modal-lg ">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="estavolonta.php" class="row w-100" method="post"  enctype="multipart/form-data">
                    <div class="row ">
                        ...
                        <div class="col-sm-6">   
                            ...
                            ?>
                            <label for="instituto" class="form-label">Nome do estagiario</label>
                        ...
                        <div class="col-sm-6">
                            ...
                            <input name="instituto" type="text" class="form-control" required>
                        </div>
                    </div>     
                    <div>
